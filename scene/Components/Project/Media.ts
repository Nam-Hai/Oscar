...
export class Media extends CanvasNode {
	...
	onScroll(e: { animatedScroll: number; velocity: number }) {
	...
	update(e: rafEvent) {
		...
		if (this.isVideo && this.uLoaded.value) {
			this.tMap.value.needsUpdate = true;
		}
	...
	onResize(canvasSize: { width: number; height: number }) {
		...
		this.bounds.y += this.fixed ? 0 : scrollLenis.value;
	...
	computeUniform() {
		...
		if (this.tMap.value.texture) {
			this.uSizePixel.value.set(this.bounds.width, this.bounds.height);
		}
