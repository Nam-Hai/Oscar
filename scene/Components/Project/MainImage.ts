import { Mesh, Plane, Program, Texture, Transform, Vec2, Vec3 } from "ogl";
...
export const [provideMainImage, useCanvasMainImageProject] =
	canvasInject<MainImage>("canvas-main-image-project");
...
export class MainImage extends CanvasNode {
	...
	constructor(
		...
		props: { borderRadius?: number; el?: HTMLElement },
	) {
		super(gl);
			...
			(b) => {
				if (b) {
					this.uniformFromTo[1].intrinsecRatio =
						(this.tMap2.value.image as HTMLImageElement).width /
						(this.tMap2.value.image as HTMLImageElement).height;
					this.uniformFromTo[2].intrinsecRatio =
						(this.tMap2.value.image as HTMLImageElement).width /
						(this.tMap2.value.image as HTMLImageElement).height;
	...
	onResize(canvasSize: { width: number; height: number }) {
		...
		this.bounds = [
			...
			this.el[1].getBoundingClientRect(),
		];
		...
		this.uniformFromTo[2].size.set(
			endBounds.value.width,
			endBounds.value.height,
		);
	...
	computeUniform() {
		...
		if (this.uProgress.value <= 1) {
			this.uSizePixel.value.set(
				N.Lerp(
					this.uniformFromTo[0].size.x,
					this.uniformFromTo[1].size.x,
					this.uProgress.value,
				),
				N.Lerp(
					this.uniformFromTo[0].size.y,
			...
			this.uScaleOffset.value.set(
				N.Lerp(
					this.uniformFromTo[0].scale.x,
					this.uniformFromTo[1].scale.x,
					this.uProgress.value,
				),
				N.Lerp(
					this.uniformFromTo[0].scale.y,
			...
			this.uTranslateOffset.value.set(
				N.Lerp(
					this.uniformFromTo[0].translate.x,
					this.uniformFromTo[1].translate.x,
					this.uProgress.value,
				),
				N.Lerp(
					this.uniformFromTo[0].translate.y,
			...
			this.uTranslateOffset.value.set(
				N.Lerp(
					this.uniformFromTo[1].translate.x,
					this.uniformFromTo[2].translate.x,
					t,
				),
				N.Lerp(
					this.uniformFromTo[1].translate.y,
					this.uniformFromTo[2].translate.y,
					t,
				),
			);
			...
			if (!this.bounds) return;
			const x =
				N.Lerp(this.bounds[1].x, 16 * scale.value, t) +
				this.uSizePixel.value.x / 2 -
				vw.value / 2;
